...
namespace Guldkortet
{
    ...
    {
        // Skapar en generisk lista för belöningar
        private MyList<Reward> rewardList = new MyList<Reward>();

        // Detta är en referens till nätverkshanteraren som sköter nätverkskommunikationen (TCP)
        ...
        public Form1()
        {
            ...
            // Skapar TcpManager och skicka med detta formulär
            tcpManager = new TcpManager(this, dbManager);
        }

        // Metod som tar emot loggtexten och skriver i serverloggen
        public void AddDebugLog(string message)
        {
            {
                // 1. Om fönstret eller textrutan håller på att stängas/är borttagen, gör ingenting
                if (IsDisposed || txtLog == null || txtLog.IsDisposed)
                {
                    return;
                }

                // 2. Om anropet kommer från fel tråd, skicka över det säkert till UI-tråden
                if (this.InvokeRequired)
                {
                    try
                    {
                        this.BeginInvoke(new Action(() => AddDebugLog(message)));
                    }
                    catch (ObjectDisposedException)
                    {
                        // Fångar upp om fönstret stängdes exakt under anropet
                    }
                    return;
                }
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            // Progress för vinster (anropar AddRewardToList automatiskt på UI-tråden)
        ...
        private async void btnUpdateKund_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                bool lyckades = await dbManager.UpdateKundAsync(användarNr, nyttNamn, nyKommun);
        ...
        private async void btnAddKund_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                bool success = await dbManager.InsertKundAsync(id, namn, kommun);
